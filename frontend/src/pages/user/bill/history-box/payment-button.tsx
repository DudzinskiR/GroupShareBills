...
const PaymentButton = ({ data, isOpen, onClick }: props) => {
  ...
  const renderWindow = () => {
    return (
      ...
      >
        <div className={`${isOpen ? "" : "select-none"} w-full h-full p-3`}>
          <div>
  ...
  return (
    <>
      <div
        ...
        } flex flex-row justify-between bg-slate-50 h-[50px]  p-1 mx-3 rounded-xl shadow cursor-pointer duration-150 relative z-1`}
        onClick={onClick}
      >
        <div className="flex flex-row items-center md:pl-3 pl-1">
          <div className="md:text-lg text-base md:max-w-[300px] sm:max-w-[250px] max-w-[150px] truncate">
            {data.description}
          </div>
          <div className="text-md italic pl-3 md:block hidden">
